...
async function setup(request, env) {
  ...
  const user = await mutateDatabase(env, async (db) => {
    if (adminCount(db) !== 0) {
      throw new ApiError(409, 'Initial setup has already been completed', 'SETUP_COMPLETE');
    }
...
async function listArticles(request, env, manage) {
  ...
  } else if (db.settings.mode === 'private' && !user) {
...
async function saveArticle(request, env, id = null) {
  ...
  const input = validateArticle(await jsonBody(request));
...
async function deleteUser(request, env, id) {
  ...
  const headers = session.uid === id ? { 'Set-Cookie': expiredSessionCookie(request) } : {};
...
async function updateSettings(request, env) {
  ...
  if (!['private', 'public'].includes(input.mode)) {
...
async function mutateDatabase(env, mutator) {
  for (let attempt = 0; attempt < 8; attempt += 1) {
    ...
    try {
      ...
      db = object ? await object.json() : createDatabase();
      validateDatabase(db);
    } catch (error) {
      if (error instanceof ApiError) throw error;
...
function validateDatabase(db) {
  if (!db || db.schemaVersion !== 1 || !isUuid(db.id)) throw new ApiError(500, 'Portal data is invalid', 'INVALID_DATABASE');
  ...
  if (!Array.isArray(db.users) || !Array.isArray(db.articles)) {
  ...
  for (const article of db.articles) {
    if (!isUuid(article.id) || !isUuid(article.authorId) || !isUuid(article.updatedById) || !ARTICLE_STATUSES.has(article.status)) {
...
function adminCount(db) {
  return db.users.filter((user) => user.role === 'administrator').length;
}
...
async function readSession(request, env) {
  ...
  if (!(await verifyHmac(payload, signature, env.SESSION_SECRET))) {
...
async function verifyPassword(password, record) {
  if (!record || record.algorithm !== 'PBKDF2-SHA-256' || !Number.isInteger(record.iterations)) return false;
  ...
  try {
    ...
    let difference = 0;
    for (let i = 0; i < bits.length; i += 1) difference |= bits[i] ^ expected[i];
    return difference === 0;
  } catch {
    return false;
  }
